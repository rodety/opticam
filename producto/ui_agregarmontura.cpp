    ...
    ui(new Ui::ui_agregarMontura)
{
    ...
    ui->comboBox_forma->ActualizarItems(forma::mostrarId());
...
void ui_agregarMontura::setMontura(montura* m)
{
    ...
    pMontura.setStock_last(pMontura.getStock());
    ...
    ui->spinBox_stock->setMinimum(pMontura.getCantidadVitrina()+pMontura.getCantidadAlmacen());
...
bool ui_agregarMontura::verificarRestricciones()
{
    ...
    if(modo == false)
    {
        ...
        query.bindValue(1,pMontura.getIdProducto());
        query.exec();
        if(query.next())
        {
...
void ui_agregarMontura::on_pushButton_agregar_clicked()
{
    ...
    //colaborador
    forma pForma;pForma.setNombre(ui->comboBox_forma->currentText());
    pForma.setIdForma(QString::number(ui->comboBox_forma->getId(ui->comboBox_forma->currentText())));
